...
unsigned long long get_uuid_52bit_20120613(int id);
int ana_key(char *p);
int ProcessRequestBuffer(redisClient *c);

unsigned long long serverid;
unsigned long long haid;
unsigned long long seqid;
unsigned long long seqidarray[UUID_BUSINESS_NUM];
unsigned long long seconds[UUID_BUSINESS_NUM];
...
/*time(23, 30bit), idc id(19, 4bit), business id(17, 2bit), seq num(2, 15bit), ha id(1, 1bit)*/
unsigned long long get_uuid_52bit_20120613(int id) {
    unsigned long long uuid= (unsigned long long)time(NULL);

	id = (id < 0 || id > UUID_BUSINESS_NUM - 1) ? 0 : id;

	//printf("for debug: uuid=%llu,seconds=%llu\n", uuid, seconds);
	if (seconds[id] != uuid){
		seconds[id] = uuid;
		seqidarray[id] = 0;
	}
    ...
    uuid-=515483463;
    uuid <<= 22;
    uuid += serverid << 18;//idc id
	uuid += id << 16;//business id
    ++ seqidarray[id];
	seqidarray[id] = seqidarray[id] % (1 << 15);
...
int ana_key(char *p){
	...
	id = (id < 0 || id > UUID_BUSINESS_NUM - 1) ? -1 : id;
	return id;
}
...
int ProcessRequestBuffer(redisClient *c) {
    ...
    if(strncasecmp(rbuf,"get ",4) == 0 ) {
		//int id = atoi(rbuf + 4);
		//printf("rbuf: %s, rid: %s, id: %d\n", rbuf, rbuf + 4, id);
		//id = (id < 0 || id > UUID_BUSINESS_NUM - 1) ? 0 : id;
		int id = ana_key(rbuf + 4);//\r\n; -1: when error
		if (id < 0 || id > UUID_BUSINESS_NUM - 1){
			return -1;
		} else {
			snprintf(c->wbuf,WRITE_BUF_LEN-1,"VALUE uuid 0 16\r\n%llu\r\nEND\r\n",get_uuid_52bit_20120613(id));
    ...
    if(strncasecmp(rbuf,"stats ",4) == 0 ) {
        snprintf(c->wbuf,WRITE_BUF_LEN-1,"STAT version %s\r\nSTAT server_id %llu\r\nSTAT ha_id %llu\r\nSTAT cmd_get %lld\r\nSTAT curr_connections %u\r\nSTAT total_connections %lld\r\nEND\r\n",
			REDIS_VERSION,serverid,haid,server.stat_numcommands,server.clients,server.stat_numconnections);
        c->wlen=strlen(c->wbuf);
        return 1;
    }
    return -1;
...
static void usage(void) {
    ...
    printf("-p <num>      TCP port number to listen on (default: 6376)\n"
           ...
           "-u <num>      the unique server id, must be a number between 0 and 15 (e.g. 12 and 13 for new business)\n"
           "-i <num>      the instance id for ha, must be 0 or 1 (default: 0)\n"
           );
    return;
}
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < UUID_BUSINESS_NUM; i ++){
		seqidarray[i] = 0;
		seconds[i] = 0;
	}
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'u':
            ...
            if (serverid < 0 || serverid > 15) {
                fprintf(stderr, "server id must be a number between 0 and 15  (e.g. 12 and 13 for new business).\n\n");
				usage();
                return 1;
            }
            break;
        case 'i':
            haid = atoi(optarg);
            if (haid != 0 && haid != 1) {
                fprintf(stderr, "ha id must be 0 or 1.\n\n");
				usage();
                return 1;
            }
			break;
    ...
    if (serverid < 0 || serverid > 15 || (haid != 0 && haid != 1)) {
        fprintf(stderr, "server id must be a number between 0 and 15(e.g. 12 and 13 for new business), and ha id must be 0 or 1.\n\n");
		usage();
        return 1;
    }
	//printf("for debug: serverid=%llu,haid=%llu,seqid=%llu\n", serverid, haid, seqid);
