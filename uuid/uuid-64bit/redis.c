...
int ProcessRequestBuffer(redisClient *c) {
    ...
    if(strncasecmp(rbuf,"get ",4) == 0 ) {
        snprintf(c->wbuf,WRITE_BUF_LEN-1,"VALUE uuid 0 19\r\n%llu\r\nEND\r\n",get_uuid());
...
static void usage(void) {
    ...
    printf("-p <num>      TCP port number to listen on (default: 6376)\n"
           ...
           "-u <num>      the unique server id, must be a number between 0 and 255\n"
           );
    return;
}
...
int main(int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'u':
            mid= atoi(optarg);
            if (mid < 0 || mid > 255) {
                fprintf(stderr, "mid must be a number between 0 and 255.\n");
                return 1;
            }
            break;
        case 'v':
